...
def divide_data():
    ...
    bound = int(0.7*len(all_set))
    bound2 = int(0.1*len(all_set))
    train_set = all_set[:bound]
    eval_set = all_set[bound:bound+bound2]
    test_set = all_set[bound+bound2:]
    print(len(train_set))
    print(len(test_set))
    pdb.set_trace()
    ...
    np.save("../data/coarse/eval_set.npy", eval_set)
...
def divide_data_ood():
    ...
    for index, row in dataset.iterrows():
        ...
        if item2knowledge[exer] in train_set_dict[student]:
            traineval_set.append([int(student), int(exer),int(score)])
    ...
    eval_set = np.array(eval_set)
    np.save("../data/coarse/eval_set_ood.npy", eval_set)
    test_set = np.array(test_set)
